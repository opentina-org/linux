...
/* TV-in / hybrid CVBS+AHD (needed by VIN CSI/MIPI paths) */
#define TVIN_CH_SIZE		4
#define TVIN_SEPARATE		4
#define TVIN_VIDEO_STRIP	4
#define TVIN_VIDEO_MAX		15
...
#define VIDIOC_TVIN_INIT \
	_IOWR('V', BASE_VIDIOC_PRIVATE + 24, struct tvin_init_info)
#define VIDIOC_SET_BK_CACHE_INVALID \
	_IOWR('V', BASE_VIDIOC_PRIVATE + 29, unsigned char)

/* Minimal Melis ISP attr stubs so headers compile without ISP server */
#ifndef ISP_AE_ROW
#define ISP_AE_ROW	16
#define ISP_AE_COL	24
#endif

struct awb_ir_gain_t {
	__u32 awb_rgain_ir;
	__u32 awb_bgain_ir;
};
...
struct isp_cfg_attr_data {
	...
	__u32 ae_ev_lv;
	__u32 ae_ev_lv_adj;
	__u32 awb_color_temp;
	__u32 ir_status;
	unsigned char ae_stat_avg[ISP_AE_ROW * ISP_AE_COL];
	struct awb_ir_gain_t awb_ir_gain;
};
