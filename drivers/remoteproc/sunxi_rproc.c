...
static struct resource_table *suxni_rproc_elf_find_loaded_rsc_table(struct rproc *rproc,
							     const struct firmware *fw)
{
	struct sunxi_rproc *chip = rproc->priv;
...
static const struct of_device_id sunxi_rproc_match[] = {
	...
	/* sun60iw2 / A733 isolated A55 AMP */
	{ .compatible = "allwinner,arm64-rproc", .data = "arm_rtos" },
	{ /* sentinel */ }
};
...
static int sunxi_rproc_resource_get(struct rproc *rproc, struct platform_device *pdev)
{
	...
	if (!chip->rproc_standby) {
		dev_dbg(dev, "no rproc standby ops for %s\n", chip->name);
	} else {
...
static int sunxi_rproc_probe(struct platform_device *pdev)
{
	...
	int ret;
	/* A733 AMP firmware is AArch64 ELF */
	u8 rproc_elf_class = ELFCLASS64;
...
EXPORT_SYMBOL(sunxi_rproc_send_mbox_msg);
