...
static SUNXI_CCU_MUX_TABLE_WITH_GATE_FEAT(osc32k_clk, "osc32k", osc32k_parents,
	NULL, 0x0, 0, 1, 0, 0, CCU_FEATURE_KEY_FIELD);

static const struct clk_parent_data dcxo24M_div32k_parent[] = {
	 { .name = "dcxo" },
};

static SUNXI_CCU_GATE_DATA_WITH_PREDIV(dcxo24M_div32k_clk, "dcxo24M-div32k",
	dcxo24M_div32k_parent, 0x60, BIT(16), 750, 0);
/*
 * rtc-1k clock
 */
...
static SUNXI_CCU_MUX_TABLE_WITH_GATE_FEAT(rtc32k_clk, "rtc32k",
	rtc32k_clk_parents, NULL, 0x0, 1, 1, 0, 0, 0);
...
static void set_reg(char __iomem *addr, u32 val, u8 bw, u8 bs)
{
	u32 mask = (1UL << bw) - 1UL;
	u32 tmp = 0;
...
static void set_reg_key(char __iomem *addr,
...
static void clock_source_init(char __iomem *base)
{
	...
	val &= ~GENMASK(15, 14);
	val |= BIT(14);
	val |= ((KEY_FIELD_MAGIC_NUM_RTC >> 16) & GENMASK(15, 0)) << 16;
...
static int sun60iw2_rtc_ccu_probe(struct platform_device *pdev)
{
	...
	return devm_sunxi_ccu_probe(&pdev->dev, reg, &sun60iw2_rtc_ccu_desc);
}
