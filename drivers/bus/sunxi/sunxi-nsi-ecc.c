...
static bool sunxi_is_support_ecc(struct platform_device *pdev)
{
	...
	if (!of_property_read_bool(np, "support-ecc")) {
		dev_dbg(&pdev->dev, "ECC disabled (support-ecc not in DT)\n");
		return false;
	}

	ret = of_property_read_u32(np, "support-ecc", &support_ecc);
	if (ret || !support_ecc) {
		dev_dbg(&pdev->dev, "ECC disabled by support-ecc\n");
		return false;
	}

	if (!sunxi_sid_get_ecc_status()) {
		dev_dbg(&pdev->dev, "ECC disabled (efuse)\n");
		return false;
	}
