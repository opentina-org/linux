...
#define IOMMU_INT_ENABLE_NO_PREFETCH 0x3003f
...
__maybe_unused static void sunxi_iommu_dump_mastger_reg(int masterid)
{
...
static void __sunxi_iommu_prevent_hang_enable_unlocked(int enable)
{
	struct sunxi_iommu_dev *iommu = global_iommu_dev;
	int i;
	/* we write nothing else but 0/1 into register */
			   ...
			   int prot, gfp_t gfp, size_t *mapped)
{
	...
	ret = sunxi_pgtable_prepare_l2_tables(sunxi_domain->pgtable,
...
static struct iommu_domain *sunxi_iommu_domain_alloc_paging(struct device *dev)
{
    ...
    if (!sunxi_domain)
        return ERR_PTR(-ENOMEM); // Modern API expects an ERR_PTR on failure
    ...
    // Modern core manages the DMA cookies for IOMMU_DOMAIN_DMA automatically.
    // If you explicitly still need it initialized here, keep it.
    ...
    sunxi_domain->domain.pgsize_bitmap = SZ_4K | SZ_16K | SZ_64K | SZ_256K |
					 SZ_1M | SZ_4M | SZ_16M;
...
static bool sunxi_iommu_skip_probe_finalize(struct device *dev)
{
	return dev->of_node &&
	       (of_device_is_compatible(dev->of_node, "allwinner,sun60i-a733-ehci") ||
		of_device_is_compatible(dev->of_node, "allwinner,sun60i-a733-ohci"));
}
...
static void sunxi_iommu_probe_device_finalize(struct device *dev)
{
	...
	if (sunxi_iommu_skip_probe_finalize(dev))
		return;
...
static bool sunxi_iommu_fault_is_prefetch(u32 inter_status, u32 bitmap)
{
	if (bitmap & (1U << 31))
		return true;

	/* PTW prefetch may only raise the L1/L2 page status bits */
	if ((inter_status & IOMMU_INT_L1PG_STA_MASK) &&
	    !(inter_status & 0xffff))
		return true;

	if ((inter_status & IOMMU_INT_L2PG_STA_MASK) &&
	    !(inter_status & 0xffff))
		return true;
					 ...
					 int index)
{
	...
	if (inter_status_reg & (1 << 0)) {
		...
		if (!sunxi_iommu_fault_is_prefetch(inter_status_reg,
						   int_masterid_bitmap))
	...
	if (!sunxi_iommu_fault_is_prefetch(inter_status_reg, int_masterid_bitmap))
					...
					struct device_node *node)
{
	...
	of_property_read_u32(node, "tlb_prefetch", &data->tlb_prefetch);
...
static const struct of_device_id sunxi_iommu_dt_ids[] = {
	{ .compatible = "allwinner,iommu-sunxi" },
	{ /* sentinel */ },
};
...
builtin_platform_driver(sunxi_iommu_driver);

MODULE_DESCRIPTION("Allwinner A733 IOMMU driver");
