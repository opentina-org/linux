...
static int sunxi_alloc_iopte(u32 *sent, int prot)
{
	...
	/*
	 * Must be GFP_ATOMIC: callers hold sunxi_domain->dt_lock.
	 * Do not pass GFP_DMA32 to kmem_cache_alloc — this cache is not a
	 * DMA slab; SLUB rejects GFP_DMA32 and warns (seen at USB OHCI probe).
	 */
	...
	iommu_p = cpu_phy_to_iommu_phy(__pa(pent));
	if (!pfn_valid(PHYS_PFN(__pa(pent))) || (iommu_p >> IOMMU_VA_BITS)) {
		phys_addr_t p = __pa(pent);

		pr_err("%s: PTE page phys %pa not representable/usable\n",
		       __func__, &p);
	...
	dma_sync_single_for_cpu(sunxi_pgtable_params.dma_dev,
				...
				DMA_TO_DEVICE);
	*sent = (u32)iommu_p | DENT_VALID;
	*sent |= ((iommu_p >> 32) << DENT_ADDR_HIGH_SHIFT) &
		 DENT_ADDR_HIGH_MASK;
...
{
	u32 *dent;
	/* iova_end is exclusive (see sunxi_iommu_map SPAGE_ALIGN). */
				    ...
				    phys_addr_t paddr, int prot)
{
	...
	for (; iova_start < iova_end;) {
		...
		dent = iopde_offset(pgtable, iova_start);
		/*
		 * Never call iopte_offset() on an invalid L1 dent: that does
		 * iommu_phy_to_cpu_phy(0) -> __va(1<<IOMMU_VA_BITS) and Oopses
		 * (seen as ffff0003c0000000 on sunxi with memstart 0x40000000).
		 */
		if (!IS_VALID(*dent) && !sunxi_alloc_iopte(dent, prot)) {
			pr_err("%s: invalid L1 dent for iova %pad (dent=0x%x)\n",
			       __func__, &iova_start, *dent);
			return -ENOMEM;
		}
		pent = iopte_offset(dent, iova_start);
		if (!virt_addr_valid(pent)) {
			pr_err("%s: bad L2 table va %px from dent 0x%x iova %pad\n",
			       __func__, pent, *dent, &iova_start);
			*dent = 0;
			if (!sunxi_alloc_iopte(dent, prot))
				return -ENOMEM;
			pent = iopte_offset(dent, iova_start);
			if (!virt_addr_valid(pent))
				return -ENOMEM;
