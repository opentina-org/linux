...
void sun55iw3_pinctrl_fix_over_voltage(bool is_cpus_gpio)
{
	...
	for (i = 0; i < banks; i++) {
		...
		if (is_cpus_gpio)
			dev_dbg(NULL, "bank[%c] power_val=%d, power_sel=%d\n", 'L' + bank, cur_uV, mod_uV);
		else
			dev_dbg(NULL, "bank[%c] power_val=%d, power_sel=%d\n", 'A' + bank, cur_uV, mod_uV);

		if (cur_uV > mod_uV) {
			if (is_cpus_gpio)
				dev_warn(NULL, "bank[%c] over voltage, we will correct it\n", 'L' + bank);
			else
				dev_warn(NULL, "bank[%c] over voltage, we will correct it\n", 'A' + bank);
			...
			if (is_cpus_gpio) {
				if (!(r_power_mon_print_once & BIT(bank))) {
					dev_warn(NULL, "bank[%c] power mode not correct, power_val=%d, power_sel=%d\n",
							'L' + bank, cur_uV, mod_uV);
...
static inline u32 sunxi_pintctrl_vccio_ctrl_map(struct sunxi_pinctrl *pctl, u32 bank)
{
	...
	for (i = 0; i < desc->vccio_nbanks; i++) {
		if (desc->vccio_banks[i] == bank) {
			dev_dbg(NULL, "vccio-ctrl-map: bank[%d] use vccio, map to bank[%d]\n",
...
static inline u32 sunxi_pintctrl_vccio_sel_map(struct sunxi_pinctrl *pctl, u32 bank)
{
	...
	for (i = 0; i < desc->vccio_nbanks; i++) {
		if (desc->vccio_banks[i] == bank) {
			dev_dbg(NULL, "vccio-sel-map: bank[%d] use vccio, map to bank[%d]\n",
...
static inline u32 sunxi_pintctrl_vccio_val_map(struct sunxi_pinctrl *pctl, u32 bank)
{
	...
	for (i = 0; i < desc->vccio_nbanks; i++) {
		if (desc->vccio_banks[i] == bank) {
			dev_dbg(NULL, "vccio-val-map: bank[%d] use vccio, map to bank[%d]\n",
					 ...
					 struct regulator *supply)
{
	...
	case BIAS_VOLTAGE_PIO_POW_MODE_CTL_V2:
		...
		dev_dbg(NULL, "!pf-withstand: bank[%d-%d]=%duV set modesel[0x%x=0x%x]\n",
		...
		raw_spin_unlock_irqrestore(&pctl->lock, flags);
		dev_dbg(NULL, "!pf-withstand: bank[%d-%d] set modectrl[0x%x=0x%x]\n",
...
static void sunxi_power_auto_switch_pf(struct sunxi_pinctrl *pctl, unsigned pin, u32 target_mV)
{
	...
	dev_dbg(NULL, "pf-switch to %d from %d\n", target_mV, current_mV);
	if (current_mV < target_mV) { /* Increase the voltage */
		...
		dev_dbg(NULL, "pf-switch-increase: Wait for voltage increase done[0x%x=0x%x] pf[0x%x=0x%x]\n",
		...
		dev_dbg(NULL, "pf-switch-decrease: Wait for voltage decrease done[0x%x=0x%x] pf[0x%x=0x%x]\n",
				power_mode_val_reg, readl(pctl->membase + power_mode_val_reg),
				pio_pow_ctrl_reg, readl(pctl->membase + pio_pow_ctrl_reg));
	...
	dev_dbg(NULL, "pf-switch to %d from %d ok\n", target_mV, current_mV);
}
static void sunxi_power_switch_pf(struct sunxi_pinctrl *pctl, unsigned pin, u32 target_mV)
{
	...
	if (current_mV < target_mV) { /* Increase the voltage */
		...
		reg &= ~(1 << bank);
		writel(reg | val << bank, pctl->membase + power_mode_sel_reg);
		dev_dbg(NULL, "pf-switch-increase: 1.Set withstand voltage value[0x%x=0x%x]\n",
		...
		reg &= ~BIT(bank);
		writel(reg | val << bank, pctl->membase + power_mode_ctrl_reg);
		dev_dbg(NULL, "pf-switch-increase: 2.Disable self-adaption[0x%x=0x%x]\n",
		...
		dev_dbg(NULL, "pf-switch-increase: 3.Wait for voltage increase done[0x%x=0x%x]\n",
			    power_mode_val_reg, readl(pctl->membase + power_mode_val_reg));
	} else if (current_mV > target_mV) { /* Decrease the voltage */
		raw_spin_lock_irqsave(&pctl->lock, flags);
		/* Decrease the voltage */
		...
		dev_dbg(NULL, "pf-switch-decrease: 1.Wait for voltage decrease done[0x%x=0x%x]\n",
			    power_mode_val_reg, readl(pctl->membase + power_mode_val_reg));
		...
		dev_dbg(NULL, "pf-switch-decrease: 2.Set withstand voltage value[0x%x=0x%x]\n",
			    power_mode_sel_reg, readl(pctl->membase + power_mode_sel_reg));
		...
		dev_dbg(NULL, "pf-switch-decrease: 3.Disable self-adaption[0x%x=0x%x]\n",
			    power_mode_ctrl_reg, readl(pctl->membase + power_mode_ctrl_reg));
	}
	dev_dbg(NULL, "pf-switch to %d from %d ok\n", target_mV, current_mV);
}
			     ...
			     unsigned group)
{
	...
	if (pctl->pad_addr[0].addr && !strncmp(func->name, pctl->pad_addr[0].name, strlen(func->name))) {
		if (g->pin > PL_BASE) {
			dev_dbg(NULL, "i2s0 change to r_pio!\n");
			writel(regval & BIT(index), pctl->pad_addr[0].addr);
		} else {
			dev_dbg(NULL, "i2s0 change to pio!\n");
			writel(regval | BIT(index), pctl->pad_addr[0].addr);
		}
	}
	...
	if (pctl->pad_addr[1].addr && !strncmp(func->name, pctl->pad_addr[1].name, strlen(func->name))) {
		if (g->pin > PL_BASE) {
			dev_dbg(NULL, "dmic change to r_pio!\n");
			writel(regval & BIT(index), pctl->pad_addr[1].addr);
		} else {
			dev_dbg(NULL, "dmic change to pio!\n");
			writel(regval | BIT(index), pctl->pad_addr[1].addr);
		}
	}
#endif
...
static int sunxi_pmx_request(struct pinctrl_dev *pctldev, unsigned offset)
{
	...
	if (pctl->desc->auto_power_source_switch) {
		dev_info_once(pctl->dev, "Auto power withstand voltage configuration detected, automatically exit!\n");
		return 0;
	}
				  ...
				  unsigned offset, int value)
{
	...
	dev_dbg(pctl->dev, "data_set_mode_select is %d", sunxi_pinctrl_hw_info[hw_type].data_set_mode_select);
	if (sunxi_pinctrl_hw_info[hw_type].data_set_mode_select) {
		...
		dev_dbg(pctl->dev, "value is %d setreg[0x%x=0x%x] clearreg[0x%x=0x%x]\n",
				    ...
				    unsigned long variant)
{
	...
	if (ret > 0) {
		...
		if (ret) {
			dev_err(&pdev->dev, "Couldn't read ignore-interrupts\n");
			ret = -ENODEV;
			goto clk_error;
		}
	} else {
	...
	for (i = 0; i < pctl->desc->irq_banks; i++) {
		...
		for (j = 0; j < array_num; j++)
			if (ignore_array[j] == irq_get_irq_data(pctl->irq[i])->hwirq) {
				pctl->ignore_irq[i] = 1;
				dev_info(&pdev->dev, "Ignore GPIO %d IRQ\n", ignore_array[j]);
			}
	}
	if (ignore_array)
	...
	if (res) {
		dev_info(&pdev->dev, "find i2s0 route contrl\n");
	...
	if (res) {
		dev_info(&pdev->dev, "find dmic route contrl\n");
	...
	dev_info(&pdev->dev, "pinctrl withstand voltage config mode=%s\n",
		withstand_auto_assert ? (pctl->desc->auto_power_source_switch ? "auto_hard" : "auto_soft") : "manual");

	dev_info_once(NULL, "sunxi pinctrl core driver version: %s\n", SUNXI_PINCTRL_CORE_VERSION);
