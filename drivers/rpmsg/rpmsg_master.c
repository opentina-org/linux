...
/*
 * Mainline rpmsg_core already registers /sys/class/rpmsg. Reuse it instead of
 * class_create("rpmsg"), which fails with -EEXIST on kernels >= 5.x/6.x.
 */
const struct class *g_aw_rpmsg_class = &rpmsg_class;
...
{
	ida_free(&rpmsg_minor_ida, MINOR(devt));
}
				...
				unsigned long arg)
{
	...
	case RPMSG_CREATE_EPT_IOCTL: {
		...
		ret = ida_alloc_min(&rpmsg_ept_ida, 1, GFP_KERNEL);
		if (ret < 0)
			return -EFAULT;
...
MODULE_DEVICE_TABLE(rpmsg, rpmsg_driver_ctrldev_id_table);
