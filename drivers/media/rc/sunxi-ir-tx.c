...
/*
 * Copyright (c) 2013-2020 Allwinner Technology Co.,Ltd.
 *
 */
/* #define DEBUG */
...
static int sunxi_irtx_regulator_request(struct sunxi_irtx *chip)
{
	...
	chip->regulator = regulator_get_optional(&chip->pdev->dev, "irtx");
	if (IS_ERR(chip->regulator)) {
		dev_dbg(&chip->pdev->dev, "no irtx regulator\n");
		chip->regulator = NULL;
	}
...
static int sunxi_irtx_resource_get(struct sunxi_irtx *chip)
{
	...
	if (IS_ERR(chip->pctrl)) {
		dev_dbg(&chip->pdev->dev, "pinctrl not available\n");
		chip->pctrl = NULL;
	}
