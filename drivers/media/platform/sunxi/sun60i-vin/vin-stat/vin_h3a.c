...
static int isp_stat_request_statistics(struct isp_stat *stat,
...
static int isp_stat_config(struct isp_stat *stat, void *new_conf)
{
	int ret;
	u32 count;
...
static int isp_stat_enable(struct isp_stat *stat, u8 enable)
{
...
static int isp_stat_subscribe_event(struct v4l2_subdev *subdev,
				  struct v4l2_fh *fh,
...
int vin_isp_h3a_init(struct isp_dev *isp)
{
	...
	snprintf(stat->sd.name, sizeof(stat->sd.name), "sunxi_h3a.%u", isp->id);
