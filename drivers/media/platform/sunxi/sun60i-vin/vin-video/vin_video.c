...
#include <linux/regulator/consumer.h>
	...
	__has_include(<linux/sunxi_dramfreq.h>)
...
static inline void dramfreq_master_access(int master, bool enable)
{
	(void)master;
	(void)enable;
}
...
static void __vin_s_stream_handle(struct work_struct *work)
{
	int ret = 0;
...
{
	strcpy(cap->driver, "sun60i-vin");
	strcpy(cap->card, "sun60i-vin");
...
static int __vin_set_fmt(struct vin_core *vinc, struct v4l2_format *f)
{
	...
	if (ret == 0) {
		...
		sel.r.width = win_cfg.width;
		sel.r.height = win_cfg.height;
		sel.r.left = win_cfg.hoffset;
		sel.r.top = win_cfg.voffset;
...
static void __osd_bitmap2dram(struct vin_osd *osd, void *databuf)
{
#if IS_ENABLED(CONFIG_ARCH_SUN8IW12P1)
	int i, j, k, m = 0, n = 0;
...
{
	/* g_pixelaspect was removed from v4l2_subdev_video_ops; default 1:1 */
	fract->numerator = 1;
	fract->denominator = 1;
	return 0;
}
				   ...
				   struct v4l2_dv_timings *timings)
{
	...
	ret = v4l2_subdev_call(cap->pipe.sd[VIN_IND_SENSOR], pad, s_dv_timings, 0, timings);
	if (ret)
				   ...
				   struct v4l2_dv_timings *timings)
{
	...
	ret = v4l2_subdev_call(cap->pipe.sd[VIN_IND_SENSOR], pad, g_dv_timings, 0, timings);
	if (ret)
		vin_err("sensor set dv timings error!\n");
				       ...
				       struct v4l2_dv_timings *timings)
{
	...
	ret = v4l2_subdev_call(cap->pipe.sd[VIN_IND_SENSOR], pad, query_dv_timings, 0, timings);
	if (ret)
...
static int vidioc_sync_ctrl(struct file *file, struct v4l2_fh *fh,
...
static int vin_init_controls(struct v4l2_ctrl_handler *hdl, struct vin_vid_cap *cap)
{
...
static int vin_init_video(struct v4l2_device *v4l2_dev, struct vin_vid_cap *cap)
{
	int ret = 0;
	struct vb2_queue *q;
