...
#define VIN_CORE_NAME "sun60i-vin-core"
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = strscpy(buf, vd_buf->data, vi_status_size_sum);
	kfree(vd_buf);
	return ret;
}
...
static void vin_get_timestamp(struct timeval *tv)
{
...
static void vin_vsync_isr(struct vin_vid_cap *cap)
{
...
static int vin_core_runtime_suspend(struct device *d)
{
	vin_log(VIN_LOG_POWER, "%s\n", __func__);
	return 0;
}
static int vin_core_runtime_resume(struct device *d)
{
	vin_log(VIN_LOG_POWER, "%s\n", __func__);
	return 0;
}
static int vin_core_runtime_idle(struct device *d)
{
	if (d) {
...
static const struct of_device_id sunxi_vin_core_match[] = {
	{.compatible = "allwinner,sun60i-vin-core",},
