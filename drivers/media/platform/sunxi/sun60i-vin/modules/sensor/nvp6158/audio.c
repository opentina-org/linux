 ...
 * extern variable
 *******************************************************************************/
extern int 				chip_nvp6158_id[4];				/* Chip ID */
extern int 				rev_nvp6158_id[4];				/* Reversion ID */
...
void nvp6168_audio_init(unsigned char recmaster, unsigned char pbmaster, unsigned char ch_num, unsigned char samplerate, unsigned char bits)
{
	...
	for (i = 0; i < nvp6158_cnt; i++) {
		...
		if (ch_num == 16) { /* 4chips' audio cascade */
			...
			gpio_i2c_write(nvp6158_iic_addr[i], 0x08, 0x01);  /* I2s outputs 4ch audio, left & right each 2 channels */
			gpio_i2c_write(nvp6158_iic_addr[i], 0x06, 0x1B);  /* rst stage */
			gpio_i2c_write(nvp6158_iic_addr[i], 0x0f, 0x32);  /* set I2S right sequence */
		}
		gpio_i2c_write(nvp6158_iic_addr[i], 0x07, 0x00 | (recmaster<<7) | (samplerate<<3) | (bits<<2));
		gpio_i2c_write(nvp6158_iic_addr[i], 0x13, 0x00 | (pbmaster<<7) | (samplerate<<3) | (bits<<2));
...
{
	if (type < NC_AD_MAX) {
		sensor_dbg("[%s] Change audio input type %d > %d\r\n", __func__, g_ai_type, type);
		g_ai_type = type;
	} else
		sensor_dbg("[%s] Invalid argument %d\r\n", __func__, type);
}
...
void audio_sample_rate_set(int sample)
{
	if (sample < NC_AD_SAMPLE_RATE_MAX) {
		sensor_dbg("[%s] Change audio sampling rate %d > %d\r\n", __func__, g_aud_sample, sample);
		g_aud_sample = sample;
	} else
		sensor_dbg("[%s] Invalid argument %d\r\n", __func__, sample);
}
...
/*
	 Set audio sampling rate
		...
		{
			sensor_dbg("[%s] Not supported yet [%d] \r\n", __func__, g_aud_sample);
		}
*/
	...
	if (g_ai_type == NC_AD_AOC) {
		...
		gpio_i2c_write(nvp6158_iic_addr[devnum], 0x58, 0x02);
...
void audio_set_aoc_format(decoder_dev_ch_info_s *decoder_info)
{
	...
	case AHD20_720P_50P:
		...
		sensor_dbg("[%s] Not supported yet. [0x%X] \r\n", __func__, decoder_info->fmt_def);
		break;
	...
	default:
		sensor_dbg("[%s] Not supported format [0x%X] \r\n", __func__, decoder_info->fmt_def);
		break;
	}
}