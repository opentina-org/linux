 ...
 *
 * Copyright (c) 2019 by Allwinner Technology Co.,Ltd.  http://www.allwinnertech.com
 *
...
*	warning			:
****************************************************************************** */
static CABLE_DISTANCE NC_VD_MANUAL_CABLE_DISTANCE_Get (unsigned char Ch, video_input_cable_dist *pDistance)
{
	...
	if ((chip_nvp6158_id[Ch/4] == NVP6168C_R0_ID) || (chip_nvp6158_id[Ch/4] == NVP6168_R0_ID))
		sMaxDistCnt = 3;
	else
		sMaxDistCnt = 10;
	...
	sensor_dbg("TESTING... Get Distance Value : ");
	for (ii = 0; ii < 6; ii++)
		sensor_dbg("[ stage: %d _ get_value: %d ]\n", ii, sGetDist[ii]);

	sensor_dbg(" Distance distinguish result : [%d]\n", sMaxGetDistVal);
...
CABLE_DISTANCE nvp6158_get_eq_dist(video_equalizer_info_s *ps_eq_info)
{
	...
	if (chip_nvp6158_id[oChannel/4] == NVP6158_R0_ID || chip_nvp6158_id[oChannel/4] == NVP6158C_R0_ID) {
		while (1) {
			...
			video_input_sam_val_read(&SAM);
			/*  NC_VD_AUTO_HSYNC_Get(oChannel, &Hsync_Accumulation);  */
			video_input_hsync_accum_read(&Hsync_Accumulation);
			/*  NC_VD_AUTO_AGC_Get(oChannel, &AGC);  */
			video_input_agc_val_read(&AGC);
			/*  NC_VD_AUTO_ACC_GAIN_Get(Ch, ACC_GAIN_NORMAL);  */
			/*  video_input_acc_gain_val_read();  */
...
static unsigned char __video_cable_manualdistance(unsigned char cabletype, video_input_hsync_accum *pvin_hsync_accum, video_input_acc_gain_val *pvin_acc_val, video_equalizer_distance_table_s *pdistance_value)
{
	int i = 0;
...
#if VIN_FALSE /*  test */
	sensor_dbg("ch[%d]: BASE, dist:%d, eq_bypass[%02x]\n", ch, dist, pbase->eq_bypass[dist]);
	sensor_dbg("ch[%d]: BASE, dist:%d, eq_band_sel[%02x]\n", ch, dist, pbase->eq_band_sel[dist]);
	sensor_dbg("ch[%d]: BASE, dist:%d, eq_gain_sel[%02x]\n", ch, dist, pbase->eq_gain_sel[dist]);
	sensor_dbg("ch[%d]: BASE, dist:%d, deq_a_on[%02x]\n", ch, dist, pbase->deq_a_on[dist]);
	sensor_dbg("ch[%d]: BASE, dist:%d, deq_a_sel[%02x]\n", ch, dist, pbase->deq_a_sel[dist]);
	sensor_dbg("ch[%d]: BASE, dist:%d, deq_b_sel[%02x]\n", ch, dist, pbase->deq_b_sel[dist]);
#endif
}
...
/*
	sensor_dbg("ch[%d]: COEFF, dist:%d, deqA_01[%02x]\n", ch, dist, pcoeff->deqA_01[dist]);
	sensor_dbg("ch[%d]: COEFF, dist:%d, deqA_02[%02x]\n", ch, dist, pcoeff->deqA_02[dist]);
	sensor_dbg("ch[%d]: COEFF, dist:%d, deqA_03[%02x]\n", ch, dist, pcoeff->deqA_03[dist]);
	sensor_dbg("ch[%d]: COEFF, dist:%d, deqA_04[%02x]\n", ch, dist, pcoeff->deqA_04[dist]);
	sensor_dbg("ch[%d]: COEFF, dist:%d, deqA_05[%02x]\n", ch, dist, pcoeff->deqA_05[dist]);
	sensor_dbg("ch[%d]: COEFF, dist:%d, deqA_06[%02x]\n", ch, dist, pcoeff->deqA_06[dist]);
	sensor_dbg("ch[%d]: COEFF, dist:%d, deqA_07[%02x]\n", ch, dist, pcoeff->deqA_07[dist]);
	sensor_dbg("ch[%d]: COEFF, dist:%d, deqA_08[%02x]\n", ch, dist, pcoeff->deqA_08[dist]);
	sensor_dbg("ch[%d]: COEFF, dist:%d, deqA_09[%02x]\n", ch, dist, pcoeff->deqA_09[dist]);
	sensor_dbg("ch[%d]: COEFF, dist:%d, deqA_10[%02x]\n", ch, dist, pcoeff->deqA_10[dist]);
	sensor_dbg("ch[%d]: COEFF, dist:%d, deqA_11[%02x]\n", ch, dist, pcoeff->deqA_11[dist]);
	sensor_dbg("ch[%d]: COEFF, dist:%d, deqA_12[%02x]\n", ch, dist, pcoeff->deqA_12[dist]);
*/
}
...
/*
	sensor_dbg("ch[%d]: COLOR, dist:%d, contrast[%02x]\n", ch, dist, pcolor->contrast[dist]);
	sensor_dbg("ch[%d]: COLOR, dist:%d, h_peaking[%02x]\n", ch, dist, pcolor->h_peaking[dist]);
	sensor_dbg("ch[%d]: COLOR, dist:%d, c_filter[%02x]\n", ch, dist, pcolor->c_filter[dist]);

	sensor_dbg("ch[%d]: COLOR, dist:%d, hue[%02x]\n", ch, dist, pcolor->hue[dist]);
	sensor_dbg("ch[%d]: COLOR, dist:%d, u_gain[%02x]\n", ch, dist, pcolor->u_gain[dist]);
	sensor_dbg("ch[%d]: COLOR, dist:%d, v_gain[%02x]\n", ch, dist, pcolor->v_gain[dist]);
	sensor_dbg("ch[%d]: COLOR, dist:%d, u_offset[%02x]\n", ch, dist, pcolor->u_offset[dist]);
	sensor_dbg("ch[%d]: COLOR, dist:%d, v_offset[%02x]\n", ch, dist, pcolor->v_offset[dist]);

	sensor_dbg("ch[%d]: COLOR, dist:%d, black_level[%02x]\n", ch, dist, pcolor->black_level[dist]);
	sensor_dbg("ch[%d]: COLOR, dist:%d, cti_delay[%02x]\n", ch, dist, pcolor->cti_delay[dist]);
	sensor_dbg("ch[%d]: COLOR, dist:%d, sub_saturation[%02x]\n", ch, dist, pcolor->sub_saturation[dist]);

	sensor_dbg("ch[%d]: COLOR, dist:%d, burst_dec_a[%02x]\n", ch, dist, pcolor->burst_dec_a[dist]);
	sensor_dbg("ch[%d]: COLOR, dist:%d, burst_dec_b[%02x]\n", ch, dist, pcolor->burst_dec_b[dist]);
	sensor_dbg("ch[%d]: COLOR, dist:%d, burst_dec_c[%02x]\n", ch, dist, pcolor->burst_dec_c[dist]);

	sensor_dbg("ch[%d]: COLOR, dist:%d, c_option[%02x]\n", ch, dist, pcolor->c_option[dist]);
*/
}
...
/*
	sensor_dbg("ch[%d]: TIMING_A, dist:%d, h_delay_a[%02x]\n", ch, dist, ptiming_a->h_delay_a[dist]);
	sensor_dbg("ch[%d]: TIMING_A, dist:%d, h_delay_b[%02x]\n", ch, dist, ptiming_a->h_delay_b[dist]);
	sensor_dbg("ch[%d]: TIMING_A, dist:%d, h_delay_c[%02x]\n", ch, dist, ptiming_a->h_delay_c[dist]);
	sensor_dbg("ch[%d]: TIMING_A, dist:%d, y_delay[%02x]\n", ch, dist, ptiming_a->y_delay[dist]);
*/
}
...
/*
	sensor_dbg("ch[%d]: TIMING_B, dist:%d, h_scaler1[%02x]\n", ch, dist, ptiming_b->h_scaler1[dist]);
	sensor_dbg("ch[%d]: TIMING_B, dist:%d, h_scaler2[%02x]\n", ch, dist, ptiming_b->h_scaler2[dist]);
	sensor_dbg("ch[%d]: TIMING_B, dist:%d, h_scaler3[%02x]\n", ch, dist, ptiming_b->h_scaler3[dist]);
	sensor_dbg("ch[%d]: TIMING_B, dist:%d, h_scaler4[%02x]\n", ch, dist, ptiming_b->h_scaler4[dist]);

	sensor_dbg("ch[%d]: TIMING_B, dist:%d, pn_auto[%02x]\n", ch, dist, ptiming_b->pn_auto[dist]);
	sensor_dbg("ch[%d]: TIMING_B, dist:%d, comb_mode[%02x]\n", ch, dist, ptiming_b->comb_mode[dist]);
	sensor_dbg("ch[%d]: TIMING_B, dist:%d, h_pll_op_a[%02x]\n", ch, dist, ptiming_b->h_pll_op_a[dist]);
	sensor_dbg("ch[%d]: TIMING_B, dist:%d, mem_path[%02x]\n", ch, dist, ptiming_b->mem_path[dist]);
	sensor_dbg("ch[%d]: TIMING_B, dist:%d, format_set1[%02x]\n", ch, dist, ptiming_b->format_set1[dist]);
	sensor_dbg("ch[%d]: TIMING_B, dist:%d, format_set2[%02x]\n", ch, dist, ptiming_b->format_set2[dist]);
	sensor_dbg("ch[%d]: TIMING_B, dist:%d, v_delay[%02x]\n", ch, dist, ptiming_b->v_delay[dist]);
*/

}

static unsigned int __get_acc_gain(unsigned char ch, unsigned char devnum)
{
...
static unsigned int __get_yplus_slope(unsigned char ch, unsigned char devnum)
{
...
static unsigned int __get_yminus_slope(unsigned char ch, unsigned char devnum)
{
...
static unsigned int __get_sync_width(unsigned char ch, unsigned char devnum)
{
	...
	while (agc_stable == 0) {
		...
		if (check_timeout++ > 100) {
			sensor_dbg(">>>>> DRV[%s:%d] CH:%d, TimeOut, AGC_stable[%x] check[%x] in get sync width\n", __func__, __LINE__, ch, reg_B0_E0, agc_stable);
			break;
		}
		msleep(1);
	}
	...
	sensor_dbg(">>>>> DRV[%s:%d] CH:%d, sync_width:0x%x\n", __func__, __LINE__, ch, sync_width);
...
int video_input_cable_measure_way(unsigned char ch, unsigned char devnum)
{
	...
	sensor_dbg(">>>>> DRV[%s:%d] CH:%d, accgain=0x%x(%d), yslope=0x%x(%d), syncwidth=0x%x(%d)\n", \
...
int nvp6158_set_equalizer(video_equalizer_info_s *pvin_eq_set)
{
	...
	if (0xFF == eq_value.eq_base.eq_band_sel[pvin_eq_set->distance] || 0x00 == eq_value.eq_base.eq_bypass[pvin_eq_set->distance]) { /* if 5x58==0xFF it's not a valid value. */
		sensor_dbg(">>>>>>DRV %s not supported video format[%2x]\n\n\n", __func__, pvin_eq_set->FmtDef);
		/*  Auto Mode ON  */
	...
	if (pvin_eq_set->FmtDef == AHD20_720P_30P_EX_Btype || pvin_eq_set->FmtDef == AHD20_720P_25P_EX_Btype ||
		...
		sensor_dbg("DRV >> This Format Support Maximum EQ Stage 10\n");
		sensor_dbg("DRV >> Now Select EQ Stage %d\n", pvin_eq_set->distance);
	} else {
		if (pvin_eq_set->distance > 5) {
			sensor_dbg("DRV >> This Format Only Support Maximum EQ Stage 5\n");
			sensor_dbg("DRV >> Now Select EQ Stage %d\n", pvin_eq_set->distance);
...
int nvp6168_set_equalizer(video_equalizer_info_s *pvin_eq_set)
{
	...
	if (0xFF == eq_value.eq_base.eq_band_sel[pvin_eq_set->distance] || 0x00 == eq_value.eq_base.eq_bypass[pvin_eq_set->distance]) { /*  if 5x58==0xFF it's not a valid value.  */
		sensor_dbg("func[%s] eq_value[fmt:%d] not found\n", __func__, pvin_eq_set->FmtDef);
		return -1;
	}
