...
/* Prebuilt libVE.so issues raw ioctl(fd, 0x804, ...). */
static_assert(IOCTL_GET_VE_TOP_REG_OFFSET == 0x804,
	      "IOCTL_GET_VE_TOP_REG_OFFSET must be 0x804 for libVE");
...
#ifdef CONFIG_VIDEO_SUN60I_CEDAR_VE_KERNEL_ENC
...
{
	/*
	 * Hardware TOP reset lives at +0x804 from the VE register base that
	 * includes the top block (decode maps 0x3000; encode uses ve2+0x800).
	 * This is independent of IOCTL_GET_VE_TOP_REG_OFFSET (0 for decode).
	 */
				   ...
				   unsigned long from_kernel)
{
	...
	switch (cmd) {
	/*
	 * Prebuilt libVE.so calls ioctl(fd, 0x804, 0). Value must match
	 * IOCTL_GET_VE_TOP_REG_OFFSET in cedar_ve_uapi.h (Tina layout).
	 */
	case IOCTL_GET_VE_TOP_REG_OFFSET: {
		return (long)cedar_devp->ve_top_reg_offset;
	}
	...
	case IOCTL_GET_ENV_INFO: {
		...
		/* IOMMU path: userspace mmaps /dev/cedar_dev*; leave address_macc 0. */
